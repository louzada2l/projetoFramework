...
const HomeAdmin = () => {
  ...
  return (
    <div className="bg-gray-100 text-green-600">
      ...
      {/* Banner Rotativo */}
      <div
        className="relative flex-col flex items-center justify-center bg-cover bg-center aspect-[16/6]"
        style={{ backgroundImage: `url(${banners[currentBanner]})` }}
      >
        <div className="absolute inset-0 bg-black opacity-60"></div>
        <h1 className="text-white relative pb-16 text-3xl">Bem-vindo ao painel de administração</h1>
      </div>
      ...
      <section className="xl:px-44 md:px-20 px-16 py-16">
        <div className="flex justify-between items-center">
          ...
          <Link
            to="/admin/add"
            className="px-6 flex items-center gap-4 text-white rounded-md bg-green-600 hover:bg-green-500"
          >
        ...
        <div className="mt-16 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 xl:gap-16 gap-8">
          {products.length === 0 ? (
            <div className="w-full text-center text-gray-600">Carregando produtos...</div>
          ) : (
